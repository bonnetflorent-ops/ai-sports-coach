...
### 1.1 Capteurs de puissance

#### Couche 1 (Débutant)

Un capteur de puissance, aussi appelé wattmètre, mesure ton effort réel en watts — c'est-à-dire la quantité d'énergie que tu produis à chaque instant sur le vélo.[^5] Contrairement à la fréquence cardiaque, la puissance n'est pas influencée par le vent, la pente, la chaleur ou ta fatigue du jour : 200 watts restent 200 watts, quelles que soient les conditions.[^5] C'est comme avoir un compteur qui mesure directement le travail que tes jambes fournissent, sans intermédiaire.[^1] Cela permet de calibrer très précisément tes séances d'entraînement et de ne pas te tromper sur l'intensité réelle de l'effort.[^3] Pour un débutant, c'est le meilleur outil pour apprendre à doser son effort sur la durée.[^5]

#### Couche 2 (Intermédiaire)

Il existe plusieurs types de capteurs selon leur emplacement : le capteur sur pédalier, le plus répandu ; le capteur intégré aux pédales, facilement interchangeable entre vélos ; et le capteur intégré au moyeu arrière.[^5] Les travaux de Gardner et al. (2004) ont démontré que les capteurs SRM et Power Tap offrent une précision et une reproductibilité élevées pour le suivi de la puissance en cyclisme, validant ces outils comme instruments de mesure fiables.[^1] Les métriques clés à connaître sont : la Normalized Power (NP), qui lisse les variations de puissance pour mieux refléter le coût physiologique réel d'une sortie avec relances ; l'Intensity Factor (IF), ratio NP/FTP qui indique l'intensité relative de la sortie ; et le Variability Index (VI), ratio NP/puissance moyenne, qui mesure l'irrégularité du pédalage.[^5] Un VI élevé sur une sortie longue indique une stratégie d'effort inefficace.[^5]

#### Couche 3 (Expert)

La calibration par zero offset est cruciale : elle permet de compenser la dérive du zéro due aux variations thermiques, particulièrement importante lors des longues sorties où l'échauffement du capteur peut introduire une erreur de plusieurs watts si non recalibrée.[^1] Sur le plan de la modélisation, le concept de Critical Power (CP) constitue la référence théorique : CP représente la puissance soutenable quasi-indéfiniment, tandis que W' (W prime) représente la capacité de travail anaérobie au-dessus de CP, une réserve épuisable.[^2] Jones et al. (2010) ont démontré les implications de ce modèle pour la détermination du VO2max et la tolérance à l'exercice.[^2] Vanhatalo et al. (2011) ont documenté les applications pratiques du modèle CP dans le sport, montrant comment il permet une caractérisation plus fine du profil de puissance de l'athlète que la simple FTP.[^3] Burnley et Jones (2018) ont approfondi la relation puissance-durée, décrivant les mécanismes physiologiques de la fatigue et les limites de la performance humaine qui sous-tendent le modèle CP/W'.[^4] La modélisation 3-paramètres (CP, W', Pmax) permet une individualisation nettement supérieure de l'entraînement par zones.[^3][^4]

***

### 1.2 Cardio, GPS et montres

#### Couche 1 (Débutant)

Une montre GPS enregistre où tu passes, à quelle vitesse, et surtout ta fréquence cardiaque (FC), qui est l'indicateur le plus simple de l'intensité de ton effort.[^5] Plus tu forces, plus ton cœur bat vite. L'important n'est pas d'avoir la FC la plus haute ou la plus basse, mais de suivre ta propre progression au fil des semaines.[^3] Ne te compare pas aux autres : certains ont un cœur qui bat naturellement plus vite, d'autres plus lentement — c'est génétique, pas un critère de performance.[^2] Ta montre te donne aussi des repères utiles au quotidien : nombre de pas, qualité de sommeil, niveau de stress.[^5] Apprends d'abord à bien connaître tes zones cardiaques personnelles avant de te perdre dans les dizaines de métriques disponibles.[^3]
...
### 1.3 Home-trainers et rouleaux

#### Couche 1 (Débutant)

Un home-trainer te permet de t'entraîner quand il pleut, quand il fait nuit, ou quand tu as 45 minutes sans pouvoir sortir.[^5] Tu fixes ton propre vélo dessus, et la résistance simule la route.[^5] En pédalant, tu produis un effort que le home-trainer mesure et ajuste.[^1] C'est l'outil idéal pour les séances structurées (intervalles, tests) car il n'y a pas de feux rouges, de descentes ou de vent pour perturber l'exercice.[^3] La précision de la mesure de puissance sur home-trainer a été validée par les mêmes principes que ceux étudiés par Gardner et al. (2004) pour les capteurs embarqués.[^1]
...
### 1.4 Analyser ses données d'entraînement

#### Couche 1 (Débutant)

Après chaque sortie, regarde seulement trois choses : la durée, la fréquence cardiaque moyenne, et ton ressenti sur une échelle de 0 à 10 (RPE — Rate of Perceived Exertion).[^5] Ces trois indicateurs simples te donnent déjà l'essentiel de l'information utile.[^3] Pas besoin de se noyer dans les graphiques au début : l'important est de repérer les tendances sur plusieurs semaines.[^5] Est-ce que la même sortie devient plus facile ? Est-ce que ta FC est plus basse pour la même allure ?[^2] C'est comme tenir un carnet de bord : la régularité de la prise de notes compte plus que la sophistication des outils.[^3]
...
### 1.5 Choisir son équipement selon budget

#### Couche 1 (Débutant)

La priorité numéro 1, ce n'est pas le capteur de puissance dernier cri ni le vélo en carbone. C'est le confort et la santé : des chaussures de running adaptées à ta foulée, remplacées tous les 800-1000 km, et une étude posturale sur le vélo pour éviter tendinites et douleurs chroniques.[^5] Ensuite, un capteur de puissance basique ou un capteur de FC suffit pour débuter.[^1] Le matériel cher ne remplace jamais l'entraînement : la régularité, le sommeil et l'alimentation priment sur l'équipement.[^3] Gardner et al. (2004) ont montré que même les capteurs d'entrée de gamme offrent une précision suffisante pour l'entraînement structuré.[^1]

#### Couche 2 (Intermédiaire)

La hiérarchie d'investissement, dans l'ordre de priorité : 1) Santé et sécurité — chaussures adaptées, casque de qualité, étude posturale vélo (l'investissement le plus rentable pour éviter des mois d'arrêt sur blessure), cuissard de qualité.[^5] 2) Capteurs de charge — un capteur de puissance est plus transformateur qu'un capteur de FC pour l'entraînement structuré, car il mesure la charge externe réelle.[^1][^5] Allen et Coggan (2010) démontrent comment l'entraînement basé sur la puissance permet une individualisation et une précision impossibles avec la seule FC.[^5] 3) Aérodynamisme — vêtements ajustés, casque aéro, position sur le vélo (le poste le plus rentable en watts gagnés par euro).[^5] 4) Poids — composants allégés, roues légères (le poste le plus coûteux pour le gain le plus faible sur le plat).[^5] Pour le vélo, le marché de l'occasion (2-3 ans, bien entretenu) offre un rapport qualité/prix imbattable.[^3]
